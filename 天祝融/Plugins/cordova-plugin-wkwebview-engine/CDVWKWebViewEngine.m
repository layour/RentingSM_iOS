...
- (id)loadRequest:(NSURLRequest*)request
{
    if ([self canLoadRequest:request]) { // can load, differentiate between file urls and other schemes
        if (request.URL.fileURL) {
            ...
            NSInteger index = [[readAccessUrl pathComponents] indexOfObject:@"www"];
            if (index != NSNotFound) {
                NSArray *omponents = [[readAccessUrl pathComponents] subarrayWithRange:NSMakeRange(0, index)];
                readAccessUrl = [NSURL fileURLWithPathComponents:omponents];
            }
            readAccessUrl = [readAccessUrl URLByDeletingLastPathComponent];
